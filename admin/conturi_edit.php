                        ...
                        <tr>
                            ...
                            <div class="Dcontainer">
								...
								if (!isset($_POST['submit']) || $_POST['submit'] == ''){ 
									...
									$cont_tip = trim($_DATA[0]['cont_tip']);
                                    ...
                                    <form method="post" id="detaliiForm" name="detaliiForm" enctype="multipart/form-data" action="<?php echo($_SERVER['REQUEST_URI']); ?>">
                                        ...
                                        <table cellpadding="2" cellspacing="2" border="0" class="w100">
	                                        ...
	                                        <tr>
												...
												?>
                                                	<table class="modules" cellspacing="1" cellpadding="5">
                                                    	<tr>
                                                        	<td></td>
														...
														<?php
                                                            for($i=0;$i<count($modules);$i++) {
																eval("$"."cur_r = $"."r_".$modules[$i]['module_id'].";"); 
																eval("$"."cur_w = $"."w_".$modules[$i]['module_id'].";"); 
                                                                if($cur_r == 1) { $cur_r_checked = ' checked="checked"'; } else { $cur_r_checked = ''; }
                                                                if($cur_w == 1) { $cur_w_checked = ' checked="checked"'; } else { $cur_w_checked = ''; }
                                                                print '
                                                                    <tr>
                                                                        <td style="color:#000;">'.$modules[$i]['module_name'].'</td>
                                                                        <td align="center"><input class="rckb" type="checkbox" name="r_'.$modules[$i]['module_id'].'" id="r_'.$modules[$i]['module_id'].'" value="1"'.$cur_r_checked.' /></td>
                                                                        <td align="center"><input class="wckb" type="checkbox" name="w_'.$modules[$i]['module_id'].'" id="w_'.$modules[$i]['module_id'].'" value="1"'.$cur_w_checked.' /></td>
                                                                    </tr>														
                                                                ';
                                                            }
                                                        ?>                                                        
                                                    </table>
                                                    <!-- WHEN WRITE PERMISSON ALWAYS ENABLE READ PERMISSION -->
                                                	<script type="text/javascript" language="javascript">
														function verifRChecked() {
														  cur_r = this.name.replace("w_", "r_");
														  cur_w = this.name;
														  if( $('#'+cur_w).is(':checked') ) { 
														  	if(!$('#'+cur_r).is(':checked') ) { 
																$('#'+cur_r).attr('checked', true);
															}
															
														  }
														}
														$('input.wckb').click(verifRChecked);
														function verifWChecked() {
														  cur_w = this.name.replace("r_", "w_");
														  cur_r = this.name;
														  if(!$('#'+cur_r).is(':checked') ) { 
														  	if($('#'+cur_w).is(':checked') ) { 
																$('#'+cur_r).attr('checked', true);
														...
														$('input.rckb').click(verifWChecked);														
													</script>
                                                </td>
                                             </tr>  
