<?php
	...
	$configArray['currentMenuSection'] = 'sesizare_edit';
	...
	$QRY = 'SELECT * FROM sesizari WHERE deleted = 0 AND sesizare_id = '.$_ID.' LIMIT 1 ';

	$_DATA = getQueryInArray($QRY, $configArray['STRUCTdbcnx']);
	if(!count($_DATA)) redirect('index.php?msg=3');
	
	
	$_LINKURI_SESIZARE = getQueryInArray("SELECT l.* FROM mm_sesizari_linkuri l WHERE l.sesizare_id = ".$_ID." AND l.link_sursa <> '' " );

	$_IMAGINI_SESIZARE = getQueryInArray("SELECT i.* FROM mm_sesizari_images i WHERE i.sesizare_id = ".$_ID." " );
	
	$_CODURI_EMBED = getQueryInArray("SELECT e.* FROM mm_sesizari_embeds e WHERE e.sesizare_id = ".$_ID." AND e.embed_sursa <> '' " );
...
?>	
                <div class="subTable">
                        ...
                        <tr>
                        	<td class="w100"><h1><img src="images/userpic_default.jpg" width="48" height="48" border="0" align="left" style="margin-right:10px;" />Modifica Sesizare</h1>
                            ...
                            <div class="Dcontainer">
								...
								if (!isset($_POST['submit']) || $_POST['submit'] == ''){ 							
									$submitOK = false;
									$sesizare_titlu = trim($_DATA[0]['sesizare_titlu']);
									$sesizare_descriere = trim($_DATA[0]['sesizare_descriere']);	
									$data_ora = substr($_DATA[0]['data_ora'], 0, -3);
									
									//Sesizare Categorii
									$selected_categs = array();
									$_CATEGS = getQueryInArray("SELECT categ_id FROM mm_categs_sesizari WHERE sesizare_id = ".$_ID."");
									for($i=0;$i<count($_CATEGS);$i++) { $selected_categs[] = $_CATEGS[$i]['categ_id'];  }
									
									//Coords
									$coord_lon = $_DATA[0]['coord_lon'];	
									$coord_lat = $_DATA[0]['coord_lat'];	
									$location_search = $_DATA[0]['location_search'];	
									$location_reverse = $_DATA[0]['location_reverse'];	
									
									//personal info
									$personal_nume = trim($_DATA[0]['personal_nume']);
									$personal_prenume = trim($_DATA[0]['personal_prenume']);
									$personal_email = trim($_DATA[0]['personal_email']);
									$personal_telefon = trim($_DATA[0]['personal_telefon']);
									
									//published
									$validated = intval($_DATA[0]['validated']);
									...
									$validated = intval($_POST['validated']);
								...
								if ($submitOK && $configArray['rightWrite']) {
							 			...
							 			$_LOG_STR = date("Y-m-d H:i:s").' '.$_SESSION['username'].' a modificat sesizarea<br /> '."\n";		
										
										$updateSesizareStr = "UPDATE sesizari SET ";
										if($_SESSION['userType'] == 'superadmin') {
											$updateSesizareStr .= "validated = '". intval($validated)."', ";
										}
										
										$updateSesizareStr .= 
											"sesizare_titlu = '". mysql_escape_string($sesizare_titlu)."', ".
											"sesizare_descriere = '". htmlspecialchars($sesizare_descriere)."', ".
											...
											"WHERE sesizare_id = ".$_ID."";
										//echo $updateSesizareStr;
										$insertErr = 0;
										if(!mysql_query($updateSesizareStr, $configArray['dbcnx'])) { $insertErr = 1; echo ' eroare la update 1: ['.mysql_error().']'; } else {  }
										...
										$insertLog = "INSERT INTO log SET ".
											...
											"ip = '".get_ip_address()."', ".
											"query = '". mysql_real_escape_string( $updateSesizareStr )."' ".
											"";								
										...
										redirect('sesizari_edit.php?id='.intval($_ID).'&msg=5');
                                        ...
                                        <table cellpadding="2" cellspacing="2" border="0" class="w100">
                                            ...
                                            <tr>
                                                ...
                                                <td align="left" valign="top" class="" style="width:100%;">
                                                    ...
                                                    <div class="clear"></div>
                                                    <br /><input name="location_search" id="location_search" type="text" placeholder="Caută adresa..." value="<?php echo(htmlspecialchars($location_search)); ?>" />
                                                    <div id="search_location_btn" class="search_location_btn"></div>
                                                    <input name="coord_lon" id="coord_lon" type="hidden" value="<?php echo(htmlspecialchars($coord_lon)); ?>" />
                                                    <input name="coord_lat" id="coord_lat" type="hidden" value="<?php echo(htmlspecialchars($coord_lat)); ?>" />                                                    
                                                </td>
                                            </tr> 
	                                        ...
	                                        <tr>
                                            	<td align="left" valign="top"><strong>Publicat:</strong></td>
                                                <td align="left" valign="top" class="error" style="width:100%;"><input type="checkbox" name="validated" <?php if($validated == 1) { print 'checked="checked"'; } ?> value="1" /><?php if(isset($error['personal_telefon'])) { echo('&nbsp; '.$error['validated']); } ?></td>
                                            </tr>                                              
                                            <?php endif; ?>
                                            <tr>
                                            	<td align="left" valign="top"><img src="images/spacer.gif" width="200" height="20" border="0" alt="" /></td>
	                                        ...
	                                        <tr>
                                                ...
                                                <td align="left" valign="top" class="" style="width:100%;"><?php echo(/*'IP: '.($_DATA[0]['added_by_ip'].' la '.*/$_DATA[0]['added_at']); ?> </td>
                                            </tr>  
	                                        <tr>
                                            	<td align="left" valign="top">Modificari:</td>
                                                <td align="left" valign="top" class="" style="width:100%; font-size:12px;"><?php echo($_DATA[0]['change_log']); ?> </td>
                                            </tr> 
	                                        <tr>
                                            	<td align="left" valign="top">Sesizari similare:</td>
                                                <td align="left" valign="top" class="" style="width:100%; font-size:12px;">
                                                <?php
													$_SIMILAR = getQueryInArray("SELECT sesizare_id, sesizare_titlu FROM sesizari WHERE deleted = 0 AND sesizare_titlu LIKE '%".$_DATA[0]['sesizare_titlu']."%' AND sesizare_id <> ".intval($_DATA[0]['sesizare_id'])." ");
													for($i=0;$i<count($_SIMILAR);$i++) {
														print '<a href="sesizari_edit.php?id='.$_SIMILAR[$i]['sesizare_id'].'">'.$_SIMILAR[$i]['sesizare_titlu'].'</a><br />';
													}//endfor $_SIMILAR
													if(!count($_SIMILAR)) { print '-'; }
												?>
												</td>
                                            </tr>                                                                                                                                                                                                                                                                                                                                                                                        
    ...
    <script type="text/javascript">
        ...
        function initEdit() {
			...
			*/
			...
			var marker_choose = [{'ID':'marker_choose', 'LATITUDE':<?php echo(htmlspecialchars($coord_lat)); ?>, 'LONGITUDE':<?php echo(htmlspecialchars($coord_lon)); ?>}];
